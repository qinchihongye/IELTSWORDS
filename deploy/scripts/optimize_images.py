...
def iter_source_images(image_dir: Path, recursive: bool):
...
def optimize_image(source_path: Path, quality: int, max_side: int, force: bool) -> tuple[bool, int, int]:
    ...
    if target_path.exists() and not force and target_path.stat().st_mtime >= source_path.stat().st_mtime:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Directory to optimize. Can be passed multiple times.",
    )
    ...
    image_dirs = args.image_dir or [root_dir / "data" / "images"]
    created = 0
    skipped = 0
    source_total = 0
    target_total = 0
    ...
    for image_dir in image_dirs:
        ...
        print(f"== {image_dir} ==")
        for source_path in iter_source_images(image_dir, recursive=args.recursive):
            ...
            if did_create:
                created += 1
                print(f"optimized: {source_path.relative_to(image_dir)} -> {source_path.with_suffix('.webp').relative_to(image_dir)}")
            else:
                skipped += 1
